...
class BinaryFraction2:
    ...
    def binary_or_not(self, N, m_min, period, jitter, b_fraction):
        """
        ...
        if foo_random_number < b_fraction:
            foo_rv, foo_err, foo_table = self.fake_binary_detection(N, m_min, period, jitter)
            return foo_table['P-value'], foo_table['Binary']
        else:
            foo_pvalue, foo_binary = self.fake_solo_detection(N, jitter)
            return foo_pvalue, foo_binary


    def Master(self, m_min, period, jitter, b_fraction, loop):
        """
        ...
        while foo_exit < loop:
            for N in range(len(self.AAS_TABLE)):
                p_value, bool_binary = self.binary_or_not(N, m_min, period, jitter, b_fraction)
                pvalue_list.append(p_value)
                binary_bool_list.append(bool_binary)
            foo_exit += 1
        return pvalue_list, binary_bool_list

    def synthetic_detection_rate(self, m_min, period, jitter, b_fraction,
                                loop):
        """
        ...
        for i in b_fraction:
            p_value_foo, binary_result_foo = self.Master(m_min, period, jitter, i, loop)
    ...
    def Synthetic_Table(self, m_min, period, jitter, loop):
        """
        ...
        """
        foo_exit = 0
        foo_rv, foo_err, final_table = self.fake_binary_detection(0,m_min, period, jitter)
        while foo_exit < loop:
            for N in range(len(self.AAS_TABLE)):
                foo_rv, foo_err, foo_table = self.fake_binary_detection(N,m_min, period, jitter)
    ...
    def _betamod(self, N, m_min, period, jitter, b_fraction):
        ...
        if binary_rng < b_fraction: # Then we're in a binary
            ...
            for n in solo_err:
                rv_foo = np.random.normal(self.AAS_TABLE['VHELIO_AVG'][N], n)
                solo_RV.append(rv_foo)
            b = self._beta(solo_RV, solo_err)
            return b
    def BetaMaster(self, m_min, period, jitter, b_fraction, loop):
        foo_exit = 0
        beta_list = []
        while foo_exit < loop:
            for N in range(len(self.AAS_TABLE)):
                one_star_beta = self._betamod(N, m_min, period, jitter, b_fraction)
                beta_list.append(one_star_beta)
            foo_exit += 1
        flat_beta = [item for sublist in beta_list for item in sublist]
        return flat_beta

    def Real_Data_Beta(self):
        """

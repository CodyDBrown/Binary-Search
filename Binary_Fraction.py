...
class Binary_Fraction:
    ...
    def buddy_values(self, N, m_min, period):
        ...
        """
        M = self.AAS_TABLE['T_MASS'][N]*u.solMass
        ...
        if P_buddy.value < 12:
            ...
            e_buddy = np.random.uniform(0.2, 0.93)*u.one
        ...
        while r_peri.value < 5*self.AAS_TABLE['T_RADIUS'][N]:
            ...
            if P_buddy.value < 12:
                ...
                e_buddy = np.random.uniform(0.2, .93)*u.one
            n = (2*np.pi) / P_buddy

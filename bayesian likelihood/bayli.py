...
def jitter(c1, c2, logg):
    ...
    foo = [(10**(c1 + c2*lg))**2 if (10**(c1 + c2*lg))**2 < 0.65 else 0.65 for lg in logg]*u.km/u.s
...
def lnprior(bf, mu, sigma):
    if 0.01 < bf < 1 and 1 < mu < 6 and 1 < sigma < 4:
        return 0
    return -np.inf


def lnprob(theta, num_of_galaxies, cloud, m_min, c1, c2):
    bf, mu, sigma = theta
    ...
    real_data_detection_rate = binary_fraction(cloud['RADIALV'],
                                               error_j)
    lp = lnprior(bf, mu, sigma)
    if not np.isfinite(lp):
        return -np.inf
    print("Done with binary fraction, ", bf, dt.datetime.now())
...
def mcmc_likelihood(gal, ndim, nwalkers, threads, nsteps, outfile,
                    c1=None, c2=None,):

    gal['RADIALV'].unit = u.km/u.s
    gal['RADIAL_ERR'].unit = u.km/u.s

    start_val = [np.array([0.5, 3.4, 2.4]) + 0.3*np.random.randn(ndim) for i in range(nwalkers)]
    ...
    sampler.run_mcmc(start_val, nsteps)
    del sampler.pool

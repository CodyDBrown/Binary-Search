...
if __name__ == "__main__":
    ...
    ndim, nwalkers = 4, 14
    pos = [np.array([0.5, 0.1, 3, 2]) + 0.3 * np.random.randn(ndim) for i in range(nwalkers)]

    threads = 4
    nsteps = 100

    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, threads=threads, args=(6, lmc, 100*u.jupiterMass, 0.61))

    sampler.run_mcmc(pos, nsteps)
    del sampler.pool
    # pickle "sampler"
    outfile = "detection_smc.pck"
